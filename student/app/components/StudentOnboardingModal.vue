...
// ── Accessibility: focus trap (shared composable) ────────────────────────────
// This is a REQUIRED intake gate (no overlay-click / no X close), so we pass NO
// onEscape — useFocusTrap then blocks Escape WITHOUT closing, keeping a keyboard
// user inside the mandatory step. It traps Tab inside the dialog (WCAG 2.4.3 /
// 2.1.2) and focuses the first field on open. Consolidated onto useFocusTrap
// (previously an inline duplicate of the same logic).
const obBox = ref<HTMLElement | null>(null)
useFocusTrap(obBox, ref(true))
</script>
...
<template>
  <Teleport to="body">
    ...
    <div class="ob-overlay">
      <div class="ob-box" ref="obBox" role="dialog" aria-modal="true" aria-label="Complete your profile">
        <div class="ob-hero">
          <div class="ob-badge">
